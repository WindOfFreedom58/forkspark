...
class LocalPlanner:
    def __init__(self, num_paths, path_offset):
        ...
        weight = 10
        self._collision_checker = CollisionChecker(circle_offsets, circle_radii, weight)

        time_gap = 1.0
        a_max = 3.5
        slow_speed = 3.0
        stop_line_buffer = 0.5
        self._velocity_planner = VelocityPlanner(time_gap, a_max, slow_speed, stop_line_buffer)
    ...
    def collision_check(self, paths, obstacles):
        return self._collision_checker.collision_check(paths, obstacles)

    def select_best_path_index(self, paths, collision_check_array, goal_state):
        return self._collision_checker.select_best_path_index(paths, collision_check_array, goal_state)