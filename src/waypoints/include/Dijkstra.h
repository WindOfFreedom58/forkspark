...
class dijkstra_exception : public std::exception {
 public:
  ...
  dijkstra_exception(int err_id) { this->err_id = err_id; }

  virtual const char *what() const throw() { return "No path found"; }

  int get_err_id() { return err_id; }
...
class Dijkstra {
 public:
  ...
  std::vector<int> findShortestPath(std::vector<std::vector<float>> *graph,
                                    int src, int target);
 ...
 private:
  std::vector<int>
      path;    // The shortest path - initialize in function getShortestPath()
  int source;  // start point

  int minDistance(std::vector<float> dist, bool sptSet[]);

  std::vector<int> getSolution(std::vector<int> parent, std::vector<float> dist,
                               int target);
